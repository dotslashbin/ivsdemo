(function () {
	...
	registerForm.addEventListener('submit', (e) => {
		...
		if(email && name) {
			...
			ajax.open('POST', `http://11.6.2.7:3000/Sign-up`, true)
	...
	showAllButton.addEventListener('click', (e) => {
		...
		e.preventDefault()
		ajax.open('GET', `http://11.6.2.7:3000/members`, true)
		ajax.responseType = 'text'
		ajax.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
		ajax.setRequestHeader(`Authorization`, `Bearer ${window.localStorage.getItem('token')}`)
		ajax.send()

		console.log(`sending getall8123`)
	})

	
	const fetchOne = (id) => {
		console.log(' posting get one ....')
		ajax.open('GET', `http://11.6.2.7:3000/members/${id}`, true)
		ajax.responseType = 'text'
		ajax.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
		ajax.setRequestHeader(`Authorization`, `Bearer ${window.localStorage.getItem('token')}`)
		ajax.send()
